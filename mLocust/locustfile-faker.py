...
try:
    ...
    coll1 = client.locust.emr_form
    coll2 = client.locust.emr_resp
    ...
    model1 = "emr_form_def.csv.500"
    model2 = "emr_resp.csv.500"
...
class MetricsLocust(User):
    ...
    ################################################################
    @task(1)
    def _bulkinsert_form(self):
        # Note that you don't pass in self despite the signature above
        tic = self.get_time();
            
        try:
            # Logic for bulk insert goes here using the pyfaker util
            l = fakerutil.bulkFetch(model1)
            coll1.insert_many(l)

            events.request_success.fire(request_type="pymongo", name="bulkinsert_form", response_time=(time.time()-tic), response_length=0)
        except Exception as e:
    ...
    @task(1)
    def _bulkinsert_resp(self):
        # Note that you don't pass in self despite the signature above
        tic = self.get_time();

        try:
            # Logic for bulk insert goes here using the pyfaker util
            l = fakerutil.bulkFetch(model2)
            coll2.insert_many(l)

            events.request_success.fire(request_type="pymongo", name="bulkinsert_resp", response_time=(time.time()-tic), response_length=0)
        except Exception as e:
            # Don't exit program. Keep going. If you want, you could call events.request_failure.fire() if you so choose.
            self.audit("exception", e)
